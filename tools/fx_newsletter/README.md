...
월요일과 금요일 아침, 원화 환율 4종을 정리한 카드뉴스 PNG를 만들어 메일로 보낸다.
...
`.github/workflows/fx-newsletter.yml`의 cron이 **일·목 23:00 UTC = 월·금 08:00 KST**에 돈다.

시간을 바꾸려면 cron 값을 고친다. GitHub Actions cron은 항상 UTC이므로 KST에서
9시간을 뺀다. 이때 요일도 하루 당겨진다는 점에 주의한다 (월요일 08:00 KST는
일요일 23:00 UTC다).

```yaml
- cron: "0 23 * * 0,4"     # 월·금 08:00 KST (현재 설정)
- cron: "0 23 * * 0"       # 월 08:00 KST — 주 1회
- cron: "0 23 * * 0,2,4"   # 월·수·금 08:00 KST
- cron: "0 22 * * 0,4"     # 월·금 07:00 KST — 지연을 감안해 한 시간 앞당김
```

> 예약 실행은 러너 부하에 따라 **수십 분에서 두 시간까지 늦게** 뜬다. 실제로
> 관측된 지연은 1시간 43분이었다. 도착 시각을 8시에 가깝게 맞추고 싶으면
> 예약을 한두 시간 앞당겨 두는 편이 실용적이다.

> GitHub은 60일간 커밋이 없는 저장소의 예약 워크플로를 자동으로 멈춘다.
> 알림이 끊기면 Actions 탭에서 다시 켜면 된다.
